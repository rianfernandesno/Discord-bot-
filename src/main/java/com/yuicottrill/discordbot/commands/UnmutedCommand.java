...
public class UnmutedCommand implements Command {
    ...
    public void execute(MessageReceivedEvent event, String[] args) {
        ...
        if (!members.isEmpty()){
            ...
            if (!event.getMember().hasPermission(Permission.BAN_MEMBERS)){
                ...
                event.getChannel().sendMessage("Você não tem permissão de desmutar usuários").queue();
            ...
            event.getGuild()
                            .removeRoleFromMember(member, mutedRole)
                                    .queue();

            event.getChannel().sendMessage("O usuário foi desmutado!").queue();
        }
    }
}
